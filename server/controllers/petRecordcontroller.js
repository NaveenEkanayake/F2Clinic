...
const countPetRecords = async(req, res) => {
    ...
    try {
        ...
        // Count the number of pet records for the given userId
...
const getPetRecordById = async(req, res) => {
    ...
    try {
        ...
        if (!isAuthorized(userRole)) {
            return res.status(403).json({
                message: "Access denied: You are not authorized to view this pet record.",
            });
        }
...
const updatePetRecord = async(req, res) => {
    ...
    try {
        ...
        if (!isAuthorized(userRole)) {
            return res.status(403).json({
                message: "Access denied: You are not authorized to update this pet record.",
            });
        }

        const updatedPetRecord = await PetRecords.findOneAndUpdate({ _id: id, UserId: userId },
...
const deletePetRecord = async(req, res) => {
    ...
    try {
        ...
        if (!isAuthorized(userRole)) {
            return res.status(403).json({
                message: "Access denied: You are not authorized to delete this pet record.",
            });
        }
